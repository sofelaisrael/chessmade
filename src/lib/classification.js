...
export function getEvaluationLossThreshold(classif, prevEval) {
  ...
  switch (classif) {
    case Classification.BEST:
      threshold =
        0.0001 * Math.pow(prevEval, 2) + 0.0236 * prevEval - 3.7143;
      break;
    case Classification.EXCELLENT:
      threshold =
        0.0002 * Math.pow(prevEval, 2) + 0.1231 * prevEval + 27.5455;
      break;
    case Classification.GOOD:
      threshold =
        0.0002 * Math.pow(prevEval, 2) + 0.2643 * prevEval + 60.5455;
      break;
    case Classification.INACCURACY:
      threshold =
        0.0002 * Math.pow(prevEval, 2) + 0.3624 * prevEval + 108.0909;
      break;
    case Classification.MISTAKE:
      threshold =
        0.0003 * Math.pow(prevEval, 2) + 0.4027 * prevEval + 225.8182;
      break;
    default:
      threshold = Infinity;
  }
